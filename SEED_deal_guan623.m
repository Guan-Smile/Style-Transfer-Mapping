...
label=[];
sqe=[];
for jj = 1:length(All_name)-1  %% 运行STM时仅保存一个session的15人（ 1/2/3 :3:length(All_name)-1），并解开下面for STM代码节的注释
...
%标准化
SourceTest_Data=zscore(SourceTest_Data);
TargetTest_Data=zscore(TargetTest_Data);
SourceTrain_Data=zscore(SourceTrain_Data);
TargetTrain_Data=zscore(TargetTrain_Data);
...
save(['seed_data/seed_625_de_LDS_zscore2',num2str(kkk),'py.mat',],...
...
% num_data =100;
% 
% imagesc(squeeze(SourceTrain_Data(3,:,:)))
% theAxes=axis;
% fmat=moviein(num_data);
% speed =10;
% for cont = 1:num_data
%     imagesc(squeeze(SourceTrain_Data(cont*speed,:,:)))
